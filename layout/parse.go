...
func (p *parser) getDirection() LayoutDirection {
	switch p.current() {
	case '[':
		return Vertical
	case '{':
...
func (p *parser) parseInnerLayout() (*Layout, error) {
	...
	} else if curr := p.current(); curr == '[' || curr == '{' {
...
func (p *parser) parseChildLayouts() ([]*Layout, error) {
	...
	layoutWithDim, err := p.parseDimentions(&Layout{})
	if err != nil {
		return nil, err
	}

	childClose, err := p.expect(',', ']', '}')
	if err != nil {
		return nil, err
	}
	if childClose == ']' && childOpen != '[' || childClose == '}' && childOpen != '{' {
		return nil, fmt.Errorf("invalid closing paren `%c` to `%c`", childClose, childOpen)
	}
	...
	if isDecimalDigit(p.current()) {
		...
		layouts = append(layouts, layoutWithDim)
		...
		layoutWithDim.Direction = p.getDirection()
		childLayouts, err := p.parseChildLayouts()
		if err != nil {
			return nil, err
		}
		layoutWithDim.Children = childLayouts
	}

	if _, err := p.expect(']', '}', 0); err != nil && !errors.Is(err, ErrParseOutOfBound) {
		return nil, err
	}
	return layouts, nil
}
